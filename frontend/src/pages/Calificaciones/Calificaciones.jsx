...
export default function Calificaciones() {
  ...
  useEffect(() => {
    if (!userCode || !isValidUserCode(userCode)) {
      setError(
        "Por favor inicia sesión con un usuario válido para ver tus calificaciones.",
      );
      setLoading(false);
      return;
    }
    ...
    fetch(`${API_URL}/api/grades/${safeUserCode}`)
      .then((r) => {
  ...
  const globalAvg =
    semesters.reduce((acc, sem) => acc + sem.average, 0) / semesters.length;
